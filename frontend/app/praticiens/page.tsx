...
const FILTERS = ['all','open','data','devops','cloud','ia','cybersecurite','mlops'];
const FILTER_LABELS: Record<string,string> = { all:'Tous', open:'● Disponibles', data:'Data', devops:'DevOps', cloud:'Cloud', ia:'IA', cybersecurite:'Cybersécurité', mlops:'MLOps' };
...
export default function PraticiensPage() {
  ...
  const filtered = praticiens.filter(p => {
    ...
    if (activeFilter !== 'all' && activeFilter !== 'open' && p.category !== activeFilter) return false;
    if (search) {
  ...
  return (
    <div style={{ padding: '4rem 6vw', maxWidth: 1200, margin: '0 auto' }}>
      <div style={{ marginBottom: '2.5rem' }}>
        ...
        <p style={{ color: 'var(--f-text-3)', fontSize: '.88rem', margin: '0 0 2rem 0' }}>Profils de praticiens tech — Data, DevOps, Cloud, IA, Cybersécurité, MLOps, Dev.</p>
      ...
      {loading ? (
        ...
        <p style={{ color: 'var(--f-text-3)', fontFamily: "'Space Mono', monospace", fontSize: '.85rem', marginTop: '2rem' }}>Aucun praticien dans cette catégorie.</p>
      ) : (
        <>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(290px,1fr))', gap: '1.25rem' }}>
              ...
              <Link key={p.slug} href={`/praticiens/${p.slug}`} className="f-card-link">
                <div className="f-card f-card-hover" style={{ display: 'flex', flexDirection: 'column', gap: '.85rem' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '1rem' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '.85rem' }}>
                      ...
                      <div>
                        ...
                        <p style={{ fontFamily: "'Space Mono', monospace", fontSize: '.68rem', color: 'var(--f-sky)', margin: '.15rem 0 0 0' }}>{p.role}</p>
                      </div>
                    </div>
                  ...
                  {p.badges?.length > 0 && (
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: '.35rem' }}>
                      {p.badges.map(b => {
                        ...
                        return <span key={b} style={{ fontFamily: "'Space Mono', monospace", fontSize: '.58rem', letterSpacing: '.08em', color: c.color, border: `1px solid ${c.border}`, background: c.bg, padding: '2px 8px', borderRadius: 4 }}>{b}</span>;
                      })}
                    </div>
                  )}
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <span style={{ fontFamily: "'Space Mono', monospace", fontSize: '.65rem', color: 'var(--f-text-3)' }}>{p.city}</span>
                    {p.open_to_work
                      ? <span style={{ fontFamily: "'Space Mono', monospace", fontSize: '.65rem', color: 'var(--f-green)', display: 'inline-flex', alignItems: 'center', gap: '.35rem' }}><span style={{ width: 6, height: 6, background: 'var(--f-green)', borderRadius: '50%', display: 'inline-block' }}></span>disponible</span>
                      : <span style={{ fontFamily: "'Space Mono', monospace", fontSize: '.65rem', color: 'var(--f-border-2)' }}>○ non dispo</span>
                    }
                  </div>
                </div>
              </Link>
          ...
          <p style={{ fontFamily: "'Space Mono', monospace", fontSize: '.65rem', color: 'var(--f-text-3)', textAlign: 'center', marginTop: '1rem' }}>
