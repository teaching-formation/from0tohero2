...
import NouvelEvenementClient from './NouvelEvenementClient';

export default async function NouvelEvenementPage() {
  const supabase = await createClient();
  const { data: { user } } = await supabase.auth.getUser();
  if (!user) redirect('/connexion?next=/mon-compte/nouvel-evenement');
  ...
  return (
    <div style={{ maxWidth: 720, margin: '0 auto', padding: '2.5rem 1.5rem' }}>
      ...
      <NouvelEvenementClient username={praticien.slug} />
    </div>
  );
}
