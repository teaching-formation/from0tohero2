...
import EditEvenementClient from './EditEvenementClient';
...
export default async function EditEvenementPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: evenement } = await supabaseAdmin
    .from('evenements').select('*').eq('id', id).maybeSingle();

  if (!evenement || evenement.praticien_id !== praticien.id) redirect('/mon-compte');
  ...
  return (
    <div style={{ maxWidth: 720, margin: '0 auto', padding: '2.5rem 1.5rem' }}>
      ...
      <EditEvenementClient evenement={evenement} />
    </div>
  );
}
