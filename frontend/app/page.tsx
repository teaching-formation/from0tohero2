  ...
  return (
    <>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(260px,1fr))', gap: '1.25rem' }}>
          {[
            ...
            <div key={s.num} className="f-card" style={{ cursor: 'default' }}>
              ...
              <span style={{ fontFamily: "'Space Mono', monospace", fontSize: '.65rem', color: s.done ? 'var(--f-green)' : 'var(--f-text-3)' }}>{s.sub}</span>
            </div>
          ))}
        </div>
      </section>
      ...
      <section style={{ padding: '5.5rem 6vw', maxWidth: 1200, margin: '0 auto' }}>
        ...
        {/* Bootcamps */}
        <p style={{ fontFamily: "'Space Mono', monospace", fontSize: '.7rem', letterSpacing: '.1em', textTransform: 'uppercase', color: 'var(--f-text-3)', marginBottom: '1rem' }}>🎓 Bootcamps</p>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(280px,1fr))', gap: '1rem', marginBottom: '3rem' }}>
          <a href="https://diakite-data.github.io/data-engineering-bootcamp/" className="f-card-link" target="_blank" rel="noreferrer">
            <div className="f-card f-card-hover">
              ...
              <div style={{ display: 'flex', gap: '1.25rem', fontFamily: "'Space Mono', monospace", fontSize: '.62rem', color: 'var(--f-text-3)', flexWrap: 'wrap' }}>
          ...
          <a href="https://da.from0tohero.dev" className="f-card-link" target="_blank" rel="noreferrer">
            <div className="f-card" style={{ opacity: .65 }}>
              <span style={{ fontFamily: "'Space Mono', monospace", fontSize: '.62rem', letterSpacing: '.1em', textTransform: 'uppercase', color: 'var(--f-text-3)', display: 'block', marginBottom: '1rem' }}>○ coming soon</span>
              ...
              <span style={{ fontFamily: "'Space Mono', monospace", fontSize: '.62rem', color: 'var(--f-text-3)' }}>En construction</span>
            </div>
          </a>
        </div>

        {/* Chaînes YouTube */}
        <p style={{ fontFamily: "'Space Mono', monospace", fontSize: '.7rem', letterSpacing: '.1em', textTransform: 'uppercase', color: 'var(--f-text-3)', marginBottom: '1rem' }}>▶ Chaînes YouTube francophones</p>
          ...
          {YOUTUBE_CHANNELS.map(ch => (
            <a key={ch.name} href={ch.url} className="f-card-link" target="_blank" rel="noreferrer">
              <div className="f-card f-card-hover" style={{ padding: '1.1rem 1.25rem' }}>
                ...
                <span style={{ fontFamily: "'Space Mono', monospace", fontSize: '.6rem', color: 'var(--f-text-3)' }}>{ch.subs}</span>
              </div>
            </a>
          ))}
        </div>
        ...
        {lastArticles.length > 0 && (
          <>
            <p style={{ fontFamily: "'Space Mono', monospace", fontSize: '.7rem', letterSpacing: '.1em', textTransform: 'uppercase', color: 'var(--f-text-3)', marginBottom: '1rem' }}>✍ Articles récents</p>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(280px,1fr))', gap: '1rem', marginBottom: '1.5rem' }}>
              {lastArticles.map(a => (
                <a key={a.slug} href={a.external_url} className="f-card-link" target="_blank" rel="noreferrer">
                  <div className="f-card f-card-hover" style={{ padding: '1.1rem 1.25rem' }}>
                    <div style={{ display: 'flex', gap: '.5rem', marginBottom: '.75rem', flexWrap: 'wrap' }}>
                      <span style={{ fontFamily: "'Space Mono', monospace", fontSize: '.6rem', letterSpacing: '.06em', textTransform: 'uppercase', color: 'var(--f-sky)', border: '1px solid var(--f-sky-border)', background: 'var(--f-sky-bg)', padding: '2px 7px', borderRadius: 2 }}>{a.category}</span>
                      <span style={{ fontFamily: "'Space Mono', monospace", fontSize: '.6rem', color: 'var(--f-text-3)', border: '1px solid var(--f-border)', padding: '2px 7px', borderRadius: 2 }}>{a.source}</span>
                    </div>
                    ...
                    <span style={{ fontFamily: "'Space Mono', monospace", fontSize: '.6rem', color: 'var(--f-text-3)' }}>{a.author} {a.author_country && `· ${a.author_country}`}</span>
                  </div>
                </a>
              ))}
            </div>
            <Link href="/articles" style={{ fontFamily: "'Space Mono', monospace", fontSize: '.72rem', color: 'var(--f-sky)', textDecoration: 'none' }}>
